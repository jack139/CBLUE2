...
def load_data(mode='train'):
    ...
    """
    if mode=='train':
        samples = data_processor.get_train_sample(dtype='num', do_augment=1)
    else:
        samples = data_processor.get_dev_sample(dtype='num')
    D = []
    for l in zip(samples['text1'], samples['label']):
...
output = Dense(
    units=data_processor.num_labels_num,
    activation='softmax',
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if val_f1 > self.best_val_f1:
            ...
            model.save_weights('num2_gp_best_model_f1_%.5f.weights'%val_f1)
        print(
...
if __name__ == '__main__':
    ...
    #model.load_weights('num2_gp_best_model_f1_0.76518.weights')
    ...
    predict_to_file('data/pos_test.json', 'pos_test.json')
