...
data_processor = CDNDataProcessor(root='./data', recall_k=RECALL_K, negative_sample=NUM_NEGATIVE_SAMPLES)
...
def load_data(mode='train'):
    ...
    """
    if mode=='train':
        samples, _, _ = data_processor.get_train_sample(dtype='cls', do_augment=DO_AUGMENT)
    else:
        samples, _, _ = data_processor.get_dev_sample(dtype='cls', do_augment=DO_AUGMENT)
    D = []
    for l in zip(samples['text1'], samples['text2'], samples['label']):
...
output = Dense(
    units=data_processor.num_labels_cls,
    activation='softmax',
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if val_f1 > self.best_val_f1:
            ...
            model.save_weights('score2_gp_best_model_f1_%.5f.weights'%val_f1)
        print(
...
if __name__ == '__main__':
    ...
    #model.load_weights('score2_gp_best_model_f1_0.91310.weights')
