...
def load_data(filename):
    ...
    for l in json.load(open(filename)):
        D.append((l['text'], category_index(l['label'])))
    return D


# 加载数据集
train_data = load_data('../dataset/CHIP-CTC/CHIP-CTC_train.json')
valid_data = load_data('../dataset/CHIP-CTC/CHIP-CTC_dev.json')
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if val_f1 > self.best_val_f1:
            ...
            model.save_weights('ctc_best_model_f1_%.5f.weights'%val_f1)
        print(
...
if __name__ == '__main__':
    ...
    model.load_weights('../ckpt/ctc_best_model_f1_0.79377.weights')
    predict_to_file('../dataset/CHIP-CTC/CHIP-CTC_test.json', 'CHIP-CTC_test.json')
