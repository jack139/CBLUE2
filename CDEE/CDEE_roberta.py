...
from undicts3 import new2text, new2value, new2tendency
...
maxlen = 512
batch_size = 4
learning_rate = 3e-6 # 6e-5, 3e-5, 1e-5, 8e-6, 5e-6
...
# 加载数据集
train_data = load_data('./data/CHIP-CDEE_train.jsonl')
valid_data = load_data('./data/CHIP-CDEE_dev.jsonl')
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if f1 >= self.best_val_f1:
            self.best_val_f1 = f1
            train_model.save_weights(f'cdee_roberta_best_f1_{f1:.5f}.weights')
...
def predict_to_file(in_file, out_file):
    ...
    new2text_k = new2text.keys()
    new2value_k = new2value.keys()
    new2tendency_k = new2tendency.keys()

    data = json.load(open(in_file))
    for d in tqdm(data, ncols=100):
        spo_list = {}
        R = set([SPO(spo) for spo in extract_spoes('[unused1]' + d['text'].upper().strip(), return_s_idx=True)])
        for e in R:
        ...
        for core_name in spo_list.keys():
            ...
            if _core_name in new2text_k: # 转换
                _core_name = new2text[_core_name][0]
            ...
            for e in spo_list[core_name]:
                if e['predicate'] == 'tendency':
                    if e['object'] in new2tendency_k: # 转换
                        e['object'] = new2tendency[e['object']][0]
                    event['tendency'] = e['object']

                elif e['predicate'] == 'character':
                    if e['object']!='[unused1]':
                        #if e['object'] in new2value_k: # 转换
                        #    e['object'] = new2value[e['object']][0]
                        event['character'].append(e['object'])

                elif e['predicate'] == 'anatomy_list':
                    if e['object']!='[unused1]':
                        #if e['object'] in new2value_k: # 转换
                        #    e['object'] = new2value[e['object']][0]
                        event['anatomy_list'].append(e['object'])
                else:
                    print('ERROR!', e)
...
if __name__ == '__main__':
    ...
    train_model.load_weights('cdee_roberta_best_f1_0.69087.weights')
    ...
    train_model.load_weights('cdee_roberta_best_f1_0.70571.weights')
    predict_to_file('../dataset/CHIP-CDEE/CHIP-CDEE_test.json', 'CHIP-CDEE_test.json')
