...
projectName = f'MedDG_生成回复_difficult_entity=={train_entity}'
if not train_entity:
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if metrics > self.best_bleu:
            ...
            train_model.save_weights(os.path.join(self.model_saved_path, 
                'MedDG_gen_difficult_%smT5_bleu_%.5f.weights'%('entity_' if train_entity else '', metrics)))  # 保存模型
...
if __name__ == '__main__':
    ...
    train_model.load_weights(os.path.join(resultPath, "MedDG_gen_difficult_mT5_bleu_0.12921.weights"))
