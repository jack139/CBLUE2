...
def load_data(filename):
    ...
    for l in json.load(open(filename)):
        D.append((l['query1'], l['query2'], int(l['label'])))
    return D


# 加载数据集
train_data = load_data('../dataset/KUAKE-QQR/KUAKE-QQR_train.json')
valid_data = load_data('../dataset/KUAKE-QQR/KUAKE-QQR_dev.json')
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if val_acc > self.best_val_acc:
            ...
            model.save_weights('qqr_gp_best_model_acc_%.5f.weights'%val_acc)
        print(
...
if __name__ == '__main__':
    ...
    model.load_weights('../ckpt/qqr_gp_best_model_acc_0.83802.weights')
    predict_to_file('../dataset/KUAKE-QQR/KUAKE-QQR_test.json', 'KUAKE-QQR_test.json')
