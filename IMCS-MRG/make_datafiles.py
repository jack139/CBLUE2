...
def deal_with_json():
    ...
    df_id_report.to_csv('data/example_report.csv',index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--chunk_size", default=500, type=int , 
                        help="chunk数据时，切分的大小")
    parser.add_argument("--example_report_file", default='data/example_report.csv', type=str, 
                        help="原始数据")
    parser.add_argument("--medi_tokenized_dir", default='data/medi_tokenized_dir', type=str, 
                        help="toknized之后的文件夹路径")
    parser.add_argument("--medi_finished_dir", default='data/medi_finished_dir', type=str, 
                        help="数据预处理完成后的文件夹路径")
